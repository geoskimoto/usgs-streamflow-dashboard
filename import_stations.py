...
def import_stations_from_csv(db_path="data/usgs_data.db"):
    ...
    # Now link stations to configurations
    print(f"\n🔗 Linking stations to configurations...")
    
    # Update Pacific Northwest Full configuration with all PNW stations
    cursor.execute("""
    UPDATE configurations 
    SET site_id = (SELECT GROUP_CONCAT(site_id) FROM stations WHERE source_dataset = 'HADS_PNW')
    WHERE config_name = 'Pacific Northwest Full'
    """)
    
    # Update Columbia River Basin with Columbia stations  
    cursor.execute("""
    UPDATE configurations
    SET site_id = (SELECT GROUP_CONCAT(site_id) FROM stations WHERE source_dataset = 'HADS_Columbia')
    WHERE config_name = 'Columbia River Basin (HUC17)'
    """)
